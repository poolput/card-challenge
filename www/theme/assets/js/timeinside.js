var fnTimeInside = {
	...
	setDateDisplay: function(){
		$('#option_group_quarter').find('input,select').attr('disabled','disabled');
		$('#option_group_year').find('input,select').attr('disabled','disabled'); 
	},
	isChangeType: function(){
		$('.series-type').off('click').on('click', function(e) {
			e.preventDefault();
			fnTimeInside.updateChart($(this).data('type'));
		});
	},
	...
	isSearch: function(){
		$('#search').off('click').on('click', function(e) {
			...
			$('#page-content').hide();
			var params = $('#criteria-form').serialize();
			var url = fnTimeInside.base_url +'/get-analysis-time-inside';
			...
			$.post(url, params, function(response){
				...
				fnTimeInside.series = JSON.parse(data.series);
				fnTimeInside.categories = data.categories.split(",");
				
				fnTimeInside.renderChart();
				fnTimeInside.renderTable(data.table);
				...
				$.post(url, params, function(response){
					if(response){
						...
						var avg_period = parseInt(total_in) / parseInt(fnTimeInside.categories.length);
	...
	renderChart: function(){
		var option = {
			   chart: {
			    ...
			    xAxis: {
					categories: fnTimeInside.categories,
			    },
			    legend: {
			        layout: 'horizontal',
			        align: 'center',
		...
		fnTimeInside.chart = Highcharts.chart('highcharts-line', option);
	},
	updateChart: function(type){
		if(type=='pie'){
			var data = [];
			for(var i =0; i< fnTimeInside.series.length; i++){
				var total_date = 0;
				for(var ii =0; ii< fnTimeInside.series[i].data.length; ii++){
					total_date = Number(total_date) + Number(fnTimeInside.series[i].data[ii]);
				}
				data[i] = {
		    		name: fnTimeInside.series[i].name,
		            y: total_date,
		            sliced: false,
					colorByPoint: true,
		            selected: true
	...
	renderTable: function(data){
		...
		for(var i =0; i< data.length; i++){
			...
			for(var ii =0; ii< fnTimeInside.categories.length; ii++){
				cnt_in = 0;
				if(data[i].column[ii]){
        ...
        $.each(fnTimeInside.categories, function(i, val){
			column_header.push({"title": val});
        	column_def.push({"targets": i+2, className: "text-center" });
        });
		if(fnTimeInside.oTable){
			fnTimeInside.oTable.destroy();
			$('#location_table').empty();
		}
		fnTimeInside.oTable = $('#location_table').DataTable({
    		"lengthChange": false,
    		"searching": false, 
    		"paging": false,
    		"bInfo" : false,
	...
	init : function(){
		fnTimeInside.isSearch();
		fnTimeInside.isChangeType();
		fnTimeInside.setDateDisplay();
	}
}
