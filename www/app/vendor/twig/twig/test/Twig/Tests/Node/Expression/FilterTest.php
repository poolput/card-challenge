...
class Twig_Tests_Node_Expression_FilterTest extends Twig_Test_NodeTestCase
{
    ...
    public function getTests()
    {
        ...
        $environment->addFilter(new Twig_SimpleFilter('bar', 'bar', array(
            'needs_environment' => true
        )));
        $environment->addFilter(new Twig_SimpleFilter('barbar', 'twig_tests_filter_barbar', array(
            'needs_context' => true,
            'is_variadic' => true
        )));
        ...
        if (function_exists('mb_get_info')) {
            ...
            $tests[] = array(
                $node,
                'twig_number_format_filter($this->env, strtoupper("foo"), 2, ".", ",")'
            );
        }
        ...
        $node = $this->createFilter($string, 'bar', array(
            new Twig_Node_Expression_Constant('bar', 1)
        ));
        $tests[] = array(
            $node,
            'bar($this->env, "abc", "bar")',
            $environment
        );
        ...
        $tests[] = array(
            $node,
            'twig_tests_filter_barbar($context, "abc")',
            $environment
        );
        ...
        $tests[] = array(
            $node,
            'twig_tests_filter_barbar($context, "abc", null, null, array("foo" => "bar"))',
            $environment
        );
        ...
        $tests[] = array(
            $node,
            'twig_tests_filter_barbar($context, "abc", null, "bar")',
            $environment
        );
        ...
        $tests[] = array(
            $node,
            'twig_tests_filter_barbar($context, "abc", "1", "2", array(0 => "3", "foo" => "bar"))',
            $environment
        );
