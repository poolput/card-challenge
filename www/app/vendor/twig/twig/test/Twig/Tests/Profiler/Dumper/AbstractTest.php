...
abstract class Twig_Tests_Profiler_Dumper_AbstractTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $profile = $this->getMockBuilder('Twig_Profiler_Profile')
        ...
        $subProfiles = array(
            $this->getIndexProfile(array(
                $this->getEmbeddedBlockProfile(),
                $this->getEmbeddedTemplateProfile(array(
        ...
        $profile->expects($this->any())
            ->method('getProfiles')
            ->will($this->returnValue($subProfiles));
        $profile->expects($this->any())
            ->method('getIterator')
            ->will($this->returnValue(new ArrayIterator($subProfiles)));
    ...
    private function generateProfile($name, $duration, $isTemplate, $type, $templateName, array $subProfiles = array())
    {
        ...
        $profile->expects($this->any())
            ->method('isTemplate')
            ->will($this->returnValue($isTemplate));
        $profile->expects($this->any())
            ->method('getType')
